...
(function () {
  ...
  // ---- the hooked photo frame ---------------------------------------------
  // The frame is the section bars' own material folded around the photo: a band
  // one section bar thick (2B), each side wearing both b-tips hooked toward the
  // exterior and, mirrored about the band's midline, toward the interior. Its
  // TILING (Jonatan's red-dot dissection on the iteration-20 BW render) is
  // twelve exact, non-overlapping pieces: 4 straight runs, cut off where each
  // hook ends, and 8 corner PRIMITIVES — at each corner the two bars' hook-ends,
  // each extended through half the corner square so the pair meets along the
  // full 45° diagonal from the joining corner J=(-B,-B) to the inner corner
  // I=(0,0) (a solid joint, not the earlier fragile point-contact), the trapped
  // inner whitespace absorbed. Each primitive has exactly 7 corners. The outer
  // notch beyond J stays open. Coords are photo-local (origin the image's
  // top-left, y down); tips in b-primitive units u = B/10.5 (bb.svg's top tip).
  // The same pieces feed the ink SVG here and the lighting in reflect.js
  // (runs profiled across the band, primitives solid, seams between pieces).
  function frameGeom(Wi, Hi, B, seam) {
    const u = B / 10.5, over = 3.9689 * u, tipX = 24.2734 * u, apexX = 21.6169 * u;
    const cut = tipX - B;                 // the run/primitive cut, from the inner corner
    const sm = seam || 0, s2 = sm / (2 * Math.SQRT2);
    // the corner primitive at the top-left corner, TOP piece (7 corners): the top
    // bar's hook-end extended to the diagonal. V5 is where the interior hook's
    // slant J->apex crosses the image edge y=0.
    const v5x = -B + apexX * (B / (B + over));
    const primT = [
      [-B + apexX, -2 * B - over],        // exterior hook apex
      [-B + tipX, -2 * B],                // hook foot on the exterior edge
      [-B + tipX, 0],                     // down the cut to the image edge
      [-B + apexX, over],                 // interior hook apex (into the photo)
      [v5x, 0],                           // back onto the image edge
      [0 + s2, 0 - s2],                   // inner corner I  (seam-inset off the diagonal)
      [-B + s2, -B - s2],                 // joining corner J (idem)
    ];
    const primB = primT.map(p => [p[1], p[0]]);         // its mirror across the diagonal
    // maps of the top-left corner onto the four corners, and the four runs
    const CORN = [
      (x, y) => [x, y], (x, y) => [Wi - x, y],
      (x, y) => [x, Hi - y], (x, y) => [Wi - x, Hi - y],
    ];
    const prims = [];
    for (const m of CORN) { prims.push(primT.map(p => m(p[0], p[1])), primB.map(p => m(p[0], p[1]))); }
    const rect = (x0, y0, x1, y1) => [[x0, y0], [x1, y0], [x1, y1], [x0, y1]];
    const runs = [
      { pts: rect(cut + sm, -2 * B, Wi - cut - sm, 0), grad: [[0, -2 * B], [0, 0]] },
      { pts: rect(cut + sm, Hi, Wi - cut - sm, Hi + 2 * B), grad: [[0, Hi + 2 * B], [0, Hi]] },
      { pts: rect(-2 * B, cut + sm, 0, Hi - cut - sm), grad: [[-2 * B, 0], [0, 0]] },
      { pts: rect(Wi, cut + sm, Wi + 2 * B, Hi - cut - sm), grad: [[Wi + 2 * B, 0], [Wi, 0]] },
    ];
    return { prims, runs };
  }

  // draw (or refresh) each photo frame's ink as an SVG inside its box — real page
  // structure, so it renders whatever the lighting is doing. The twelve pieces
  // tile the frame exactly, so the ink is just their seamless fills.
  function buildFrame() {
    for (const ph of document.querySelectorAll(".about__photo")) {
      ...
      const g = frameGeom(W - 2 * T2, H - 2 * T2, T2 / 2, 0);
  ...
  window.LITAPI = { incRGB, glowFalloff, GOLD, GLOW_FRAC, TEXT_SEL, frameGeom,
