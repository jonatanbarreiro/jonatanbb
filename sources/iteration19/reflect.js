...
(function () {
  ...
  const GOLD = "#fad02a";     // the official site gold (--gold): clicked/selected words

  // ---------- WebGL ----------
  let gl = null, glcv = null, mcv = null, mc = null, tex = null, prog = null, glok = true, prepped = false;
  const uni = {}; let words = [], bars = [], tips = [], frames = [], lastKey = "";
  ...
  function prep() {
    ...
    for (const k of document.querySelectorAll(".kicker")) {
      ...
      const xL = last.right + sx + 0.4 * fs, xR = kr.right + sx;
    ...
    // The photo frame: four MITRE-CUT sides (the y=±x diagonals through the corners),
    // each a trapezoid lit across its own thickness — 4 normals per side. A hairline
    // seam along each diagonal keeps the corners reading as joints, like the b's.
    for (const ph of document.querySelectorAll(".about__photo")) {
      const r = ph.getBoundingClientRect(), t = parseFloat(getComputedStyle(ph).borderTopWidth) || 0;
      if (t < 2) continue;
      const x = r.left + sx, y = r.top + sy, w = r.width, h = r.height, g = SEAM;
      frames.push(
        { pts: [[x+g, y], [x+w-g, y], [x+w-t-g, y+t], [x+t+g, y+t]],          grad: [0, y, 0, y+t] },
        { pts: [[x+g, y+h], [x+t+g, y+h-t], [x+w-t-g, y+h-t], [x+w-g, y+h]],  grad: [0, y+h, 0, y+h-t] },
        { pts: [[x, y+g], [x+t, y+t+g], [x+t, y+h-t-g], [x, y+h-g]],          grad: [x, 0, x+t, 0] },
        { pts: [[x+w, y+g], [x+w, y+h-g], [x+w-t, y+h-t-g], [x+w-t, y+t+g]],  grad: [x+w, 0, x+w-t, 0] });
    }
    prepped = true;
  }
  function ensureSize() {
  ...
  function raster(dpr, sx, sy) {
    ...
    // the photo frame's mitre-cut sides: each profiled across its own thickness
    for (const f of frames) {
      const y0 = Math.min(f.pts[0][1], f.pts[2][1]) - sy, y1 = Math.max(f.pts[0][1], f.pts[2][1]) - sy;
      if (y1 < -20 || y0 > innerHeight + 20) continue;
      const [gx0, gy0, gx1, gy1] = f.grad;
      mc.fillStyle = shoulders(mc.createLinearGradient(
        gx0 ? gx0 - sx : 0, gy0 ? gy0 - sy : 0, gx1 ? gx1 - sx : 0, gy1 ? gy1 - sy : 0));
      poly(f.pts); mc.fill();
    }
    // the selection outline: a 3px ink stroke, lit like the words it wraps
    mc.strokeStyle = "#fff"; mc.lineWidth = 3;
    for (const s of selRects) {
      const y = s.y - sy; if (y + s.h < -20 || y > innerHeight + 20) continue;
      mc.beginPath();
      if (mc.roundRect) mc.roundRect(s.x - sx + 1.5, y + 1.5, s.w - 3, s.h - 3, 3.5);
      else mc.rect(s.x - sx + 1.5, y + 1.5, s.w - 3, s.h - 3);
      mc.stroke();
    }
    mc.fillStyle = "#fff";
  ...
  let selRects = [];          // the outline boxes, page coords — rasterised as ink too
  ...
  function updateSelection() {
    ...
    if (sel && sel.rangeCount && !sel.isCollapsed) {
      ...
      for (const L of lines) {
        ...
        selRects.push({ x: L.left + sx - 3, y: L.top + sy - 1, w: L.right - L.left + 6, h: L.bottom - L.top + 2 });
      }
    } else selRects = [];
    applyHighlights();
