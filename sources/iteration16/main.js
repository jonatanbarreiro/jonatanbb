...
(function () {
  ...
  function sizeCanvas() {
    ...
    // keep the letters' backing store at ~3× device px whatever the display, so the small
    // carved message stays smoothly antialiased instead of jagged
    const lettersSS = Math.max(1, Math.ceil(3 / (window.devicePixelRatio || 1)));
    ...
    if (lctx) fitCanvas(lettersCanvas, lctx, 0, lettersSS);  // letters: supersampled, no blur — crisp
