...
(function () {
  ...
  window.LITAPI = { incRGB, glowFalloff, GOLD, TEXT_SEL, frameGeom, pointMesh,
  ...
  // Every LITERALLY-gold word is a light source of its own (window.LITGOLD from reflect.js:
  // the CSS-gold copy plus any clicked/selected word). Its glow is an ELLIPSE at HALF the
  // points' reach — r₋/2 across the short axis, λ·r₋/2 along the long one (λ = the word's
  // own elongation) — so a wide section label throws its light sideways onto the rule bar
  // beside it. Reflective lighting from these same sources is added in reflect.js.
  const GOLD_GLOW = GOLD;                          // gold words glow in their own gold
  function drawGoldGlow(cx, cy, rOut, rIn, sx, sy) {
    if (!cctx || !window.LITGOLD || !window.LITGOLD.length) return;
    const vw = castCanvas._cw, vh = castCanvas._ch, half = rIn / 2;
    cctx.save();
    clipOutLogo(cctx, cx, cy, rOut);
    const gg = cctx.createRadialGradient(0, 0, 0, 0, 0, 1);   // one unit-space gradient, reused under each word's transform
    gg.addColorStop(0, `rgba(${GOLD_GLOW[0]},${GOLD_GLOW[1]},${GOLD_GLOW[2]},0.40)`);
    gg.addColorStop(1, `rgba(${GOLD_GLOW[0]},${GOLD_GLOW[1]},${GOLD_GLOW[2]},0)`);
    cctx.fillStyle = gg;
    for (const o of window.LITGOLD) {
      const px = o.cx - sx, py = o.cy - sy;
      const lam = o.ry > 0.5 ? o.rx / o.ry : 1;               // the word's elongation
      const a = lam * half, b = half;                          // reach: r₋/2 short, λ·r₋/2 long
      if (px + a < 0 || px - a > vw || py + b < 0 || py - b > vh) continue;
      cctx.save(); cctx.translate(px, py); cctx.scale(a, b);
      cctx.beginPath(); cctx.arc(0, 0, 1, 0, TAU); cctx.fill(); cctx.restore();
    }
    cctx.restore();
  }
  ...
  function draw() {
    ...
    drawGoldGlow(cx, cy, rOut, rIn, sx, sy);      // gold glow from every literally-gold word
    if (phase === "gather")
    ...
    // its full wake w, out to r₋ (glowR). reflect.js adds the gold words' own half-reach lighting
    // from its LITGOLD geometry — it just needs r₋ (fr.rIn) to size them.
  ...
  function layoutDots() {
    ...
    buildCracks();                                    // the SPT lightning follows the field
  }

  // ---- the point mesh: Delaunay over the field, pruned, plus its tree ------
  // Bowyer–Watson triangulation of the dots' page coordinates, then:
  //  • every side on the mesh's outer perimeter is removed;
  //  • per triangle, ONLY its longest side is removed, and only when it is more
  //    than TWICE the triangle's shortest (a side shared by two triangles goes
  //    if either triangle condemns it);
  //  • over what survives, the shortest-path tree rooted at the i-dot is marked
  //    (Dijkstra, edge weight = length).
  // Returns every Delaunay edge in page px: [{ax, ay, bx, by, kept, tree}].
  function pointMesh() {
    const idx = [], pts = [];
    for (let i = 0; i < dotPos.length; i++)
      if (dotPos[i]) { idx.push(i); pts.push([dotPos[i].x, dotPos[i].y]); }
    const n = pts.length;
    if (n < 3) return [];
    const root = idx.indexOf(iIndex);      // the SPT root: the dot on the 'i'
    // a super-triangle comfortably enclosing the field
    ...
    for (let p = 0; p < n; p++) {
      ...
      for (const [a, b] of edges) {          // the cavity's boundary: edges seen exactly once
        let dup = 0;
    ...
    for (const t of tris) {
      if (t.v.some(v => v >= n)) continue;                     // touches the super-triangle
      const L = [0, 1, 2].map(e => elen(t.v[e], t.v[(e + 1) % 3]));
      const mn = Math.min(L[0], L[1], L[2]), mx = Math.max(L[0], L[1], L[2]);
      for (let e = 0; e < 3; e++) {
        const a = t.v[e], b = t.v[(e + 1) % 3], k = ekey(a, b);
        all.set(k, [a, b]);
        seen.set(k, (seen.get(k) || 0) + 1);
        if (mx > 2 * mn && L[e] === mx) cut.add(k);            // only the triangle's LONGEST side goes
      }
    }
    for (const [k, c] of seen) if (c === 1) cut.add(k);        // the mesh's perimeter goes first
    // the shortest-path tree from the i-dot over the surviving sides (Dijkstra)
    const adj = Array.from({ length: n }, () => []);
    for (const [k, [a, b]] of all)
      if (!cut.has(k)) { adj[a].push(b); adj[b].push(a); }
    ...
    if (root >= 0) {
      ...
      for (;;) {
        ...
        for (const v of adj[u]) {
          const d = dist[u] + elen(u, v);
          if (d < dist[v]) { dist[v] = d; par[v] = u; }
        }
      }
      for (let v = 0; v < n; v++) if (par[v] >= 0) tree.add(ekey(v, par[v]));
    }
    const out = [];
    for (const [k, [a, b]] of all)
      out.push({ ax: pts[a][0], ay: pts[a][1], bx: pts[b][0], by: pts[b][1],
                 kept: !cut.has(k), tree: tree.has(k) });
    return out;
  }

  // ---- the cracks: the field's shortest-path tree, revealed by the pulse ----
  // The pulse's energy runs the SPT rooted at the i-dot; where it passes, the page
  // cracks. Each tree edge is a LIGHTNING filament — a midpoint-displacement fractal
  // (a strong spine with a faint offshoot or two), seeded from its endpoints so it is
  // stable across frames and rebuilds. The front reveals each filament OUTWARD from
  // the root: a point on it lights once the front has reached its distance from the
  // i-dot, and stays lit after. Invisible before the pulse (crackR is 0 through the
  // gather); full and permanent once the wave has cleared (crackR = Infinity).
  let crackPaths = [];      // [{ pts:[[x,y]…] page coords, dr:[dist-from-root…], core }]
  let crackR = 0;           // revealed radius from the i-dot (page px); Infinity = all shown
  function seededRng(seed) {
    let s = seed >>> 0;
    return () => { s = (s + 0x6D2B79F5) | 0; let t = Math.imul(s ^ (s >>> 15), 1 | s);
      t = (t + Math.imul(t ^ (t >>> 7), 61 | t)) ^ t; return ((t ^ (t >>> 14)) >>> 0) / 4294967296; };
  }
  // A to B by midpoint displacement: recursively offset each midpoint perpendicular
  // to its segment, halving the amplitude per level — jagged, mostly linear, irregular.
  // amp0 is the first offset as a fraction of the span.
  function filament(ax, ay, bx, by, rng, levels, amp0) {
    let pts = [[ax, ay], [bx, by]];
    let amp = Math.hypot(bx - ax, by - ay) * amp0;
    for (let lv = 0; lv < levels; lv++) {
      ...
      np.push(pts[pts.length - 1]); pts = np; amp *= 0.5;
    }
    return pts;
  }
  function pushFilament(pts, rootP, core) {
    crackPaths.push({ pts, core, dr: pts.map(p => Math.hypot(p[0] - rootP.x, p[1] - rootP.y)) });
  }
  function buildCracks() {
    crackPaths = [];
    const rootP = dotPos[iIndex]; if (!rootP) return;
    for (const e of pointMesh()) {
      if (!e.tree) continue;                                   // only the SPT edges crack
      const dA = Math.hypot(e.ax - rootP.x, e.ay - rootP.y);
      const dB = Math.hypot(e.bx - rootP.x, e.by - rootP.y);
      const [x0, y0, x1, y1] = dA <= dB ? [e.ax, e.ay, e.bx, e.by] : [e.bx, e.by, e.ax, e.ay];
      const seed = (Math.round(x0) * 73856093) ^ (Math.round(y0) * 19349663)
                 ^ (Math.round(x1) * 83492791) ^ Math.round(y1);
      const rng = seededRng(seed);
      pushFilament(filament(x0, y0, x1, y1, rng, 5, 0.05), rootP, 1);   // the spine — hugs the straight join
      const L = Math.hypot(x1 - x0, y1 - y0);
      for (let b = 0, nb = L > 90 ? 2 : L > 45 ? 1 : 0; b < nb; b++) {  // faint offshoots — no new node, kept SHORT
        const sp = crackPaths[crackPaths.length - 1 - b].pts;
        const j = 2 + Math.floor(rng() * Math.max(1, sp.length - 4));
        const a = sp[j], nx = sp[Math.min(j + 1, sp.length - 1)];
        const dir = Math.atan2(nx[1] - a[1], nx[0] - a[0]) + (rng() < 0.5 ? -1 : 1) * (0.5 + rng() * 0.5);
        const bl = L * (0.07 + rng() * 0.07);
        pushFilament(filament(a[0], a[1], a[0] + Math.cos(dir) * bl, a[1] + Math.sin(dir) * bl, rng, 3, 0.12), rootP, 0.5);
      }
    }
  }
  // Paint the revealed cracks: every filament up to crackR, in the site gold with a
  // hot core; the growing tips (within a lead of the front) flash brighter. Its own
  // fixed layer, so it holds registration on scroll like the other canvases.
  function drawCracks(scale) {
    ...
    const sx = window.scrollX, sy = window.scrollY, lead = ign.lead || 80;
    const spine = new Path2D(), branch = new Path2D(), front = new Path2D();
    let any = false;
    for (const cp of crackPaths) {
      const { pts, dr } = cp, P = cp.core >= 1 ? spine : branch;
      for (let i = 0; i < pts.length - 1; i++) {
        const da = dr[i], db = dr[i + 1];
        ...
        P.moveTo(a[0] - sx, a[1] - sy); P.lineTo(b[0] - sx, b[1] - sy);
        if (crackR !== Infinity && Math.max(da, db) > crackR - lead) { front.moveTo(a[0] - sx, a[1] - sy); front.lineTo(b[0] - sx, b[1] - sy); }
        any = true;
      }
    }
    if (!any) return;
    const g = GOLD, cw = 0.6 + 0.28 * scale;   // thin — the illuminating points must read clearly bigger
    krctx.lineCap = "round"; krctx.lineJoin = "round";
    krctx.strokeStyle = `rgba(${g[0]},${g[1]},${g[2]},0.14)`;                 // halo
    krctx.lineWidth = cw * 2.4; krctx.stroke(spine);
    krctx.lineWidth = cw * 1.6; krctx.stroke(branch);
    krctx.strokeStyle = `rgba(${g[0]},${g[1]},${g[2]},0.85)`;                 // gold body
    krctx.lineWidth = cw; krctx.stroke(spine);
    krctx.lineWidth = cw * 0.66; krctx.stroke(branch);
    krctx.strokeStyle = "rgba(255,244,206,0.9)";                             // hot core
    krctx.lineWidth = cw * 0.5; krctx.stroke(spine);
    krctx.strokeStyle = "rgba(255,250,230,0.95)";                           // the growing front, brighter
    krctx.lineWidth = cw * 1.1; krctx.stroke(front);
  }
  ...
  function onMove(e) {
    ...
    if (drag.kind === "dot") {
      ...
      buildCracks();                         // the tree (and its cracks) re-form around the moved dot
    } else {
