...
// point (and the gold words' elliptical glow), painted on the cast canvas and clipped out of the
// logo disc. This file is the other layer: REFLECTIVE light — the points (and, at half reach, the
// gold words) actually shading the page ink gold. It is deliberately independent of the logo
// (main.js hands it the point field un-gated), so a point near the logo still lights nearby copy.
//
...
(function () {
  ...
  // is a computed colour the site gold (#fad02a)? — tells LITERALLY-gold copy (the
  // section labels, the contact head, a clicked word) from ink merely lit gold by
  // the shader, whose DOM colour stays dark ink
  function colorIsGold(str) {
    const m = str && str.match(/\d+/g); if (!m || m.length < 3) return false;
    return Math.abs(+m[0] - 250) < 22 && Math.abs(+m[1] - 208) < 22 && Math.abs(+m[2] - 42) < 22;
  }
  ...
  const MAXL = 24;   // shader light slots (points reaching r₋ + gold-word sources at r₋/2)
  ...
  function prep() {
    ...
    for (const s of document.querySelectorAll(".litw")) {
      ...
      words.push({ el: s, text: s.textContent, tt: cs.textTransform, font,
        ...
        bot: r.bottom + sy, blk: blockOf(s),    // line + block, for the reflective boxes
        gold: !s.classList.contains("goldw") && colorIsGold(cs.color),   // CSS-gold copy (not click/selection gold — that is tracked live)
    ...
    for (const k of document.querySelectorAll(".kicker")) {
      ...
      const cY = kr.top + sy + kr.height / 2;               // the bar's centreline (centred on the kicker's line)
    ...
    rebuildGold();                          // the gold light-sources (main.js reads window.LITGOLD)
    prepped = true;
  ...
  window.TEXT_LIGHT = function (lights, f) {
    ...
    // the light list the shader reads: the point field (each reaching r₋, its passed r)
    // PLUS the gold words' own sources — multi-circle at HALF reach (r₋/2), laid along each
    // word's long axis so a wide label lights the rule bar beside it. All circular, so the
    // shader stays a plain point-light loop. Gold guarded off when the master light is off.
    const src = [];
    for (const L of lights) src.push({ x: L.x, y: L.y, r: L.r, i: L.i });
    if (!lightsOff() && f.rIn && window.LITGOLD) {
      const half = f.rIn / 2;
      for (const o of window.LITGOLD) {
        const gx = o.cx - f.sx, gy = o.cy - f.sy;
        const lam = o.ry > 0.5 ? o.rx / o.ry : 1, a = lam * half;
        const m = Math.max(1, Math.round(a / half));           // circles along the long axis
        for (let j = 0; j < m; j++) {
          const t = m === 1 ? 0 : (j / (m - 1)) * 2 - 1;        // -1..1 across the word
          src.push({ x: gx + t * (a - half), y: gy, r: half, i: 0.9 });
        }
      }
    }
  ...
  function applyHighlights() {
    ...
    rebuildGold();                                      // the click/selection gold changed the source set
    window.dispatchEvent(new Event("scroll"));          // ask main.js to repaint the glow
  }
  // The gold light-sources main.js glows from (window.LITGOLD): LITERALLY-gold words
  // only — the CSS-gold copy (section labels, the contact head, the pub/lang tags,
  // baked as w.gold at prep) plus any word turned gold by a click or the selection.
  // NOT every reflectively-lit word. One entry per whole word (its data-g group),
  // reduced to a centre + half-extents so a single elliptical source covers it.
  function rebuildGold() {
    const grp = new Map();
    for (const w of words) {
      if (!(w.gold || clicked.has(w.el) || selected.has(w.el))) continue;
      const g = w.el.dataset.g || w.el;
      const b = grp.get(g) || { x0: Infinity, y0: Infinity, x1: -Infinity, y1: -Infinity };
      b.x0 = Math.min(b.x0, w.px); b.y0 = Math.min(b.y0, w.top);
      b.x1 = Math.max(b.x1, w.px + w.w); b.y1 = Math.max(b.y1, w.bot);
      grp.set(g, b);
    }
    const out = [];
    for (const b of grp.values())
      out.push({ cx: (b.x0 + b.x1) / 2, cy: (b.y0 + b.y1) / 2, rx: (b.x1 - b.x0) / 2, ry: (b.y1 - b.y0) / 2 });
    window.LITGOLD = out;
  }
