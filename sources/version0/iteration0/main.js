...
(function () {
  ...
  const KLOGO = 1.21833;                            // logo scale (fills 95% of inner radius)
  const HW = 10.5 * KLOGO / 2;                      // half stroke, viewBox units
  const gX = x => 200 + KLOGO * (x - 81.8);         // the group transform (MEC-centred)
  const gY = y => 200 + KLOGO * (y - 137.775);
  ...
  function draw() {
    ...
    // the thickened circumference: outer/inner boundary radii (the black ring is
    // stroke-width 13.65 in the 0..400 viewBox, i.e. half = 6.825 each side)
    const halfStroke = 6.825 * scale, rOut = R + halfStroke, rIn = R - halfStroke;
