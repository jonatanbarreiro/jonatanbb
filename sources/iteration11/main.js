...
(function () {
  ...
  // holds its glow a moment, then falls off so it's fully gone ~6s after illumination — a
  // slow ember fade. (The b faces no longer cool — they light only, see below.)
  const COOL_TAU = 2.46, COOL_BETA = 1.6;
  ...
  function drawCast(ext, cx, cy, rOut, rIn) {
    ...
    for (const q of ext) {
      ...
      const reach = q.d - rIn;                  // stop at the inner boundary — the cast never enters the logo
      wedge(q.px, q.py, outer.am, outer.half, reach, CAST_GLOW, CAST_ALPHA * 0.45);              // wide, aims at the outer boundary
      if (inner) wedge(q.px, q.py, inner.am, inner.half, reach, CAST_GLOW, CAST_ALPHA * 1.40);   // middle, aims at the inner boundary
      wedge(q.px, q.py, outer.am, outer.half * 0.30, reach, CAST_WHITE, CAST_ALPHA * 1.45);      // narrow white flashlight, on top
    }
  }
