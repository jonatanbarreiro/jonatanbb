...
(function () {
  ...
  const mbbs = [];        // reflective elements' bounding boxes (page coords), cached by layoutDots
  ...
  // A point only focuses cones once it has something CLOSE to aim at — the logo, or
  // (iteration 20) the reflective page copy. With nothing within reach it is just a
  // bulb: a tight 360° radial glow. "Close" is 2·rIn everywhere, the annulus' inner
  // diameter; q.gate is the logo's own ramp (full by gap rIn, gone at 2·rIn) — the
  // same band over which the point illuminates the logo at all. q.w is how awake it is.
  ...
  window.LITAPI = { incRGB, glowFalloff, GOLD, GLOW_FRAC, TEXT_SEL, frameGeom, pointMesh,
  ...
  function drawCast(ext, cx, cy, rOut, rIn, scale, sx, sy) {
    if (!cctx) return;
    cctx.clearRect(0, 0, castCanvas._cw, castCanvas._ch);
    const glowR = GLOW_FRAC * scale, reach2 = 2 * rIn;
    ...
    // a true circular sector (the text cones can merge wider than a triangle covers)
    const sector = (ex, ey, a0, a1, rch, c, alpha) => {
      if (!(a1 > a0) || !(rch > 0)) return;
      const g = cctx.createRadialGradient(ex, ey, 0, ex, ey, rch);
      g.addColorStop(0, `rgba(${c[0]},${c[1]},${c[2]},${alpha.toFixed(3)})`);
      g.addColorStop(1, `rgba(${c[0]},${c[1]},${c[2]},0)`);
      cctx.fillStyle = g;
      cctx.beginPath(); cctx.moveTo(ex, ey); cctx.arc(ex, ey, rch, a0, a1); cctx.closePath(); cctx.fill();
    };
    for (const q of ext) {
      const w = q.w == null ? 1 : q.w; if (w <= 0) continue;
      const coneW = q.gate == null ? 1 : q.gate;
      // ---- the glowing cones onto the page COPY (iteration 20) ----
      // Each reflective element within 2·rIn (its minimal bounding box, by boundary
      // distance — the same "close" the logo uses) subtends an angular interval at
      // the point: the span of its box corners. The point cones over the UNION of
      // those intervals; INSIDE a box there is no direction — it glows radially.
      let inside = false, textW = 0; const ivs = [];
      const tgts = window.LITBOXES || mbbs;    // reflect.js' tight line boxes, when present
      for (const b of tgts) {
        const x0 = b.x0 - sx, y0 = b.y0 - sy, x1 = b.x1 - sx, y1 = b.y1 - sy;
        const ddx = Math.max(x0 - q.px, 0, q.px - x1), ddy = Math.max(y0 - q.py, 0, q.py - y1);
        if (!ddx && !ddy) { inside = true; break; }
        const dB = Math.hypot(ddx, ddy); if (dB >= reach2) continue;
        const bw = Math.min(1, 2 - dB / rIn); if (bw <= 0.01) continue;
        const am = Math.atan2((y0 + y1) / 2 - q.py, (x0 + x1) / 2 - q.px);
        let lo = 0, hi = 0, far = 0;
        for (const [vx, vy] of [[x0, y0], [x1, y0], [x1, y1], [x0, y1]]) {
          const d = Math.hypot(vx - q.px, vy - q.py); if (d > far) far = d;
          let da = Math.atan2(vy - q.py, vx - q.px) - am;
          da = Math.atan2(Math.sin(da), Math.cos(da));       // signed offset from the box's centre ray
          if (da < lo) lo = da; if (da > hi) hi = da;
        }
        if (bw > textW) textW = bw;
        ivs.push({ lo: am + lo, hi: am + hi, w: bw, far: Math.min(far, reach2), am, half: (hi - lo) / 2 });
      }
      if (inside) { ivs.length = 0; textW = 0; }   // inside an element's box: radial mode
      const glowW = 1 - Math.max(coneW, textW);
      if (GLOW_SCALE > 0 && glowW > 0.01) {        // no near target: a soft omni glow around the point
      ...
      if (ivs.length) {                            // the cones onto the copy it sees
        ivs.sort((a, b2) => a.lo - b2.lo);
        const merged = [];
        for (const iv of ivs) {
          const m = merged[merged.length - 1];
          if (m && iv.lo <= m.hi + 0.02) {
            m.hi = Math.max(m.hi, iv.hi); m.w = Math.max(m.w, iv.w); m.far = Math.max(m.far, iv.far);
          } else merged.push({ lo: iv.lo, hi: iv.hi, w: iv.w, far: iv.far });
        }
        if (merged.length > 1) {                   // circular wrap: last meets first
          const f = merged[0], l = merged[merged.length - 1];
          if (l.hi >= f.lo + TAU - 0.02) { f.lo = l.lo - TAU; f.w = Math.max(f.w, l.w); f.far = Math.max(f.far, l.far); merged.pop(); }
        }
        for (const m of merged)                    // main beam over the union interval
          sector(q.px, q.py, m.lo, m.hi, m.far, CAST_GLOW, CAST_ALPHA * 1.30 * m.w * w);
        for (const iv of ivs)                      // brighter core toward each element itself
          wedge(q.px, q.py, iv.am, iv.half * 0.55, iv.far, CAST_GLOW, CAST_ALPHA * 0.85 * iv.w * w);
      }
      if (coneW > 0.01) {                          // near the logo: the focused cones
  ...
  function draw() {
    ...
    for (let i = 0; i < dotEls.length; i++) {
      ...
      // the annulus' INNER DIAMETER, the same "close" the text cones use (a point
      // entering the bowl sees the whole interior). Full by gap = rIn, gone at 2·rIn;
      // gates the cast cones and the illumination alike.
    ...
    drawCast(ext, cx, cy, rOut, rIn, scale, sx, sy);
    if (phase === "gather")
      drawPreambleGlow(icx, icy, gatherProg, glowR,
        Math.hypot(vw, document.documentElement.scrollHeight));   // page diagonal
    // UNIFIED ILLUMINATION: the logo's own lighting — the boundary bands, the b
    // faces and their pulse response — is no longer painted here. The logo's ink
    // is rasterised into the reflective layer's coverage mask (reflect-unified.js
    // reads fr.logo below) and lit by the same shader as the page copy. The
    // reflect canvas only needs clearing; the ring letters keep their own heat.
    if (ctx) ctx.clearRect(0, 0, reflectCanvas._cw, reflectCanvas._ch);
    ...
    if (window.TEXT_LIGHT) {
      ...
      // UNIFIED: the logo joins the reflective layer's mask — its live geometry,
      // viewport px (the signet is fixed; it turns and drags, so all of it goes)
      fr.logo = { cx, cy, rIn, rOut, rot: rotRad, scale };
  ...
  function layoutDots() {
    ...
    // the reflective elements' minimal bounding boxes (page coords), for the glowing
    // cones the points throw at nearby copy — coarse on purpose: one box per text
    // block / kicker row / photo frame / contact icon
    mbbs.length = 0;
    if (window.LITAPI) {
      const els = window.LITAPI.targets()
        .concat([...document.querySelectorAll(".about__photo, .contact__icon svg")]);
      for (const el of els) {
        const r = el.getBoundingClientRect();
        if (r.width < 2 || r.height < 2) continue;
        mbbs.push({ x0: r.left + window.scrollX, y0: r.top + window.scrollY,
                    x1: r.right + window.scrollX, y1: r.bottom + window.scrollY });
      }
    }
