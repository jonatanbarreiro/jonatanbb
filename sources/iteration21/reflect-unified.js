// jonatanbb.xyz — REFLECTIVE lighting of the page copy — UNIFIED variant: the logo
// (annulus + b's) is rasterised into this same mask and lit by the same shader,
// replacing main.js' bespoke band/element painting (see the UNIFIED comments below)
...
(function () {
  ...
  let logo = null;   // UNIFIED: the logo's live geometry (from main-unified.js), rasterised into the mask
  ...
  const FRAG =
    ...
    // UNIFIED: the mask's red channel tags the ink — text is drawn white (r=a),
    // the logo black (r=0). Logo pixels see each light over a LONGER reach
    // (its old engagement range outlived the text's), same shading otherwise.
    "  float tag = clamp(texture2D(tex, uv).r / a, 0.0, 1.0);" +
    "  float rk = mix(1.75, 1.0, tag);" +
    "  vec2 px = uv * res;" +
    "  float e = 1.4;" +
    ...
    "    float R = lr[k]*rk;" +
    "    vec2 d = lp[k]-px; float dist=length(d); if(dist>R) continue;" +
    "    float fall = 1.0 - dist/R; fall*=fall;" +
  ...
  function prep() {
    ...
    for (const k of document.querySelectorAll(".kicker")) {
      ...
      // past where the straight run is inset for the tips) in as a box so the cones target
      // it WITH the heading. Live geometry, no baked positions.
    ...
    window.LITBOXES = buildBoxes(sx, sy);   // the cones' targets (main.js reads it)
    prepped = true;
    lastKey = "";   // a fresh cache always re-rasters — even if scroll/canvas look unchanged
  }

  // ---------- the reflective boxes: tight line groups, for the cones ----------
  // The glowing layer aims its cones at these (window.LITBOXES, page coords).
  ...
  function raster(dpr, sx, sy) {
    ...
    // UNIFIED: the logo is mask ink too, in viewport coords (the signet is fixed —
    // it doesn't scroll, it TURNS with scroll; the transform carries the live
    // rotation). The annulus is profiled across its thickness like a bar run; the
    // b's are their own line-art, a soft wide pass under a solid core so the
    // ribbon shows stroke-like shoulders to the normal-reading shader.
    // Drawn BLACK: the red channel tags logo ink for the shader (text ink is
    // white), which reads its lights over a longer reach — see FRAG.
    if (logo) {
      const rg = mc.createRadialGradient(logo.cx, logo.cy, logo.rIn, logo.cx, logo.cy, logo.rOut);
      rg.addColorStop(0, "rgba(0,0,0,0)"); rg.addColorStop(0.28, "#000");
      rg.addColorStop(0.72, "#000");       rg.addColorStop(1, "rgba(0,0,0,0)");
      mc.fillStyle = rg;
      mc.beginPath();
      mc.arc(logo.cx, logo.cy, logo.rOut, 0, 2 * Math.PI);
      mc.arc(logo.cx, logo.cy, logo.rIn, 0, 2 * Math.PI, true);
      mc.fill();
      const bPath = new Path2D("M27.5 86.5 21 93 V58 L49.5 86.5 0 136 V1.445 L4.856 8.7");
      mc.strokeStyle = "#000"; mc.lineJoin = "miter"; mc.lineCap = "butt"; mc.miterLimit = 6;
      for (const t of [[44.6, 81.8], [119, 57.75]]) {       // the two b's USE translates
        mc.save();
        mc.translate(logo.cx, logo.cy); mc.rotate(logo.rot);
        mc.scale(logo.scale * 0.835096, logo.scale * 0.835096);
        mc.translate(t[0] - 107.637, t[1] - 168.3);
        mc.globalAlpha = 0.45; mc.lineWidth = 10.5; mc.stroke(bPath);
        mc.globalAlpha = 1;    mc.lineWidth = 6.8;  mc.stroke(bPath);
        mc.restore();
      }
      mc.globalAlpha = 1;
    }
  ...
  window.TEXT_LIGHT = function (lights, f) {
    ...
    // proportional right-and-down gold drift of the zoom captures. The logo's
    // position and rotation join it: a dragged logo moves without any scroll.
    const key = f.sx + ":" + f.sy + ":" + glcv.width + ":" + glcv.height + ":" + dpr + ":" + innerWidth +
      (logo ? ":" + logo.cx.toFixed(1) + ":" + logo.cy.toFixed(1) + ":" + logo.rot.toFixed(4) + ":" + logo.rOut.toFixed(1) : ":x");
