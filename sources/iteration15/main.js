...
(function () {
  ...
  const FONT_FRAC = 0.35;             // letter font px as a fraction of the ring thickness (nudged up a tad — still subtle)
  ...
  const ES = {
    ...
    lvl_native: "nativo",
    lvl_fluent: "fluido",
    lang_line: "Español y gallego nativos, inglés fluido.",
    chead: "Hablemos",
  ...
  if (langBtn) {
    ...
    if (recall("lang") === "es") setLang("es");             // restore a remembered language
  }

  // the bulb belongs to the top of the page — fade it out once you scroll into the content;
  // the language switch stays put (CSS keeps it visible) so it can still be reached
  function syncControls() {
    if (controls) controls.classList.toggle("controls--scrolled", window.scrollY > 160);
  }
