...
(function () {
  ...
  // each point lights up as the announcer reaches it, is full at the main front,
  // holds to the requiem, then blinks dark and does its own bulb, coming online as a
  // real light whose glow and logo-light track the bulb. The logo (each b element,
  // each ring arc) responds the same way, timed from the i-dot. When the pulse has
  // left the screen the i-dot bulbs back on from black, and steady light takes over.
  ...
  const IGN = {
    ...
    I_BULB: 0.95,      // the i-dot's closing bulb (s)
    P_BULB: 0.80,      // a point's own bulb, once the requiem has cleared it (s)
    FADE_TAU: 1.20, FADE_BETA: 1.5,   // the logo's ease-off after the requiem
  };
  ...
  // The pulse envelope a logo element feels, by its distance r from the i-dot: dark
  // before the announcer reaches it, ramping to full by the main front, held to the
  // requiem, then faded out on the requiem's own tail. This dictates only the element's
  // TIMING and level — its direction comes from where the i-dot sits, applied where the
  // element is painted. lead is the announcer's head start; nothing before the release.
  function pulseMul(r) {
    ...
    if (Tp < rT) return 1;
    return Math.exp(-Math.pow((Tp - rT) / IGN.FADE_TAU, IGN.FADE_BETA));
  }
  ...
  // holds its glow a moment, then falls off so it's fully gone ~5s after illumination — a
  // slow ember fade. (The b faces no longer cool — they light only, see below.)
  const COOL_TAU = 2.05, COOL_BETA = 1.6;
  ...
  const FONT_FRAC = 0.42;             // letter font px as a fraction of the ring thickness (smaller again this round)
  ...
  // The stylized pulse response of the b's. As the wave sweeps the monogram, each element
  // lights on the one face turned toward the i-dot. Its timing is keyed to the nearer of
  // that face's two endpoints (the first the wave reaches): start at the announcer, full at
  // the main front, held to the requiem, faded with it — all from pulseMul(rE). The level is
  // the envelope; the direction is which faces face the i-dot (and how square-on). Painted
  // over the physical b-light, so awakening points blend in seamlessly as the wave passes.
  function paintPulseB(cx, cy, scale, cR, sR, ix, iy) {
    if (!ctx) return;
    const scl = KLOGO * scale;
    for (let b = 0; b < 2; b++) {
      const t = USES[b];
      for (let el = 0; el < 18; el++) {
        ...
        let lx = ix - (A[0]+B[0])/2, ly = iy - (A[1]+B[1])/2; const L = Math.hypot(lx, ly) || 1;
        const facing = (N[0]*lx + N[1]*ly) / L; if (facing <= 0) continue;   // face turned away from the i-dot
        const rE = Math.min(Math.hypot(ix-A[0], iy-A[1]), Math.hypot(ix-B[0], iy-B[1]));
        const e = pulseMul(rE) * facing;                                     // envelope, shaped by how square-on it is
        if (e < 0.02) continue;
        paintFace(A, B, N, new Array(11).fill(e), F.p, scl, 1);
      }
    }
  }
  ...
  function sizeCanvas() {
    ...
    if (lctx) fitCanvas(lettersCanvas, lctx, 0.18 * s);      // letters, the lightest diffusion (crisper still)
  ...
  // needed) — so only interior points ever light inside, which also calms the cast over the
  // page content. The three, bottom -> top:
  //  • a WIDE one, edges tangent to the OUTER boundary — a subtle base glow;
  //  • a MIDDLE one, edges tangent to the INNER boundary — the main theme glow;
  ...
  // (set in draw from the point's distance to the logo): pure glow at three outer
  // radii away, pure cones by two, crossfading between — the same band over which the
  ...
  function drawCast(ext, cx, cy, rOut, rIn, scale) {
    ...
    for (const q of ext) {
      ...
      const glowW = 1 - coneW;
      if (glowW > 0.01) {                          // far: a soft omni glow around the point
        const a = (CAST_ALPHA * 1.6 * glowW * w);
        const g = cctx.createRadialGradient(q.px, q.py, 0, q.px, q.py, glowR);
        g.addColorStop(0, `rgba(${CAST_GLOW[0]},${CAST_GLOW[1]},${CAST_GLOW[2]},${a.toFixed(3)})`);
        g.addColorStop(1, `rgba(${CAST_GLOW[0]},${CAST_GLOW[1]},${CAST_GLOW[2]},0)`);
        cctx.fillStyle = g;
      ...
      if (coneW > 0.01) {                          // near: the focused cones, growing in as the glow fades
        ...
        wedge(q.px, q.py, outer.am, outer.half, reach, CAST_GLOW, CAST_ALPHA * 0.60 * cw);              // wide, aims at the outer boundary
        if (inner) wedge(q.px, q.py, inner.am, inner.half, reach, CAST_GLOW, CAST_ALPHA * 1.85 * cw);   // middle, aims at the inner boundary
  ...
  function drawPreambleGlow(icx, icy, prog, glowR, Rpage) {
    ...
    const g = cctx.createRadialGradient(icx, icy, 0, icx, icy, radius);
    g.addColorStop(0, `rgba(${CAST_GLOW[0]},${CAST_GLOW[1]},${CAST_GLOW[2]},${peak.toFixed(3)})`);
    g.addColorStop(1, `rgba(${CAST_GLOW[0]},${CAST_GLOW[1]},${CAST_GLOW[2]},0)`);
    cctx.fillStyle = g;
  ...
  // holds a moment and fades out by ~6s. Sets `lettersWarm` while any letter is still
  ...
  function draw() {
    ...
    let phase = "steady";                  // gather | pulse | close | steady
    ...
    if (igReady) {
      ...
      ign.Tp = T - pulseT0;
      const delta = IGN.TRAIL / IGN.SPEED;                       // the requiem is born this long after the main front
      const finalStartT = pulseT0 + (Rmax + IGN.TRAIL) / IGN.SPEED;   // requiem clears the far corner
      const Tf = T - finalStartT;
      let maxDot = 0;                                            // farthest point from the i-dot (for the last bulb)
      for (let i = 0; i < dotPos.length; i++) {
        if (i === iIndex || !dotPos[i]) continue;
        const d = Math.hypot(dotPos[i].x - sx - icx, dotPos[i].y - sy - icy);
        if (d > maxDot) maxDot = d;
      }
      const endT = Math.max(finalStartT + IGN.I_BULB,
                            pulseT0 + (maxDot + IGN.TRAIL) / IGN.SPEED + IGN.P_BULB);
      ...
      } else if (T < pulseT0) {                                  // GATHER: warm black -> gold along the annulus path
        phase = "gather"; gatherProg = T / IGN.PREAMBLE;
        setLit(iIndex, gatherProg); setCore(incandescent(gatherProg)); iW = 0;
      } else if (Tf >= 0) {                                      // CLOSE: pulse gone, the i-dot bulbs back from black
        phase = "close"; iW = bulb(Tf / IGN.I_BULB);
        setLit(iIndex, iW); setCore("");
      } else {                                                   // PULSE: freed; the i-dot dumps its light and goes dark
        phase = "pulse"; iW = 0;
        if (ign.Tp < delta) {                                    // most yellow gone at release, black by the requiem
          const drain = 0.18 * (1 - ign.Tp / delta);
          setLit(iIndex, drain); setCore(incandescent(drain));
        } else { setLit(iIndex, 0); setCore(""); }               // the dark source while the pulse travels
      }
    }
    const pulseLogo = igReady && igOn && ign.Tp >= 0;            // the wave is lighting the logo (incl. fading tails)
    ...
    for (let i = 0; i < dotEls.length; i++) {
      ...
      const gate = Math.max(0, Math.min(1, 4 - dist / rOut));   // logo engagement: full within 3·rOut, gone by 4·rOut

      let w = 1;                              // steady state: every point fully awake
      if (igOn) {
        if (!igReady) { w = 0; }              // before the clock starts: all dark
        else if (i === iIndex) { w = iW; }    // the i-dot's --lit/core is set above; it lights only when closing/steady
        else if (ign.Tp < 0) { setLit(i, 0); w = 0; }   // still gathering: every other point dark
        else {
          const r = Math.hypot(px - icx, py - icy), leadT = ign.lead / IGN.SPEED;
          const aT = (r - ign.lead) / IGN.SPEED, mT = r / IGN.SPEED, rT = (r + ign.trail) / IGN.SPEED, Tp = ign.Tp;
          let lit;
          if (Tp >= rT)      lit = bulb((Tp - rT) / IGN.P_BULB);   // requiem cleared -> its own bulb
          else if (Tp >= mT) lit = 1;                              // full from the main front to the requiem
          else if (Tp >= aT) lit = (Tp - aT) / (leadT || 1);       // rising from the announcer
          else               lit = 0;                              // the wave hasn't reached it yet
          setLit(i, lit);
          w = lit;                            // the light tracks the bulb/glow (req 4)
        }
      }
    ...
    // the two boundary-illumination sequences over the uniform arc partition. During
    // the pulse each small arc also lights as the wave sweeps it: keyed to the arc
    // point's distance from the i-dot, it rises at the announcer, peaks at the main
    // front, holds to the requiem, fades with it — taken as the brighter of the two.
    ...
    for (let k = 0; k < N; k++) {
      ...
      if (pulseLogo) {
        const eo = pulseMul(Math.hypot(icx - (cx + rOut*ca), icy - (cy + rOut*sa)));
        const ei = pulseMul(Math.hypot(icx - (cx + rIn*ca),  icy - (cy + rIn*sa)));
        if (eo > to) to = eo;
        if (ei > ti) ti = ei;
      }
  ...
  // b-primitive point-in-silhouette test the lighting uses — see illumination.txt).
