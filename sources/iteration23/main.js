...
(function () {
  ...
  (function scatter() {
    ...
    // margins included, on purpose — they decorate a wide screen), but the density is set
    // so the PAGE CONTENTS get a bit MORE than the ~17 points they'd hold if all of them
    // fell on the content column (max-width 1080px). On a wide screen that means more
    // points overall — which keeps the lightning connections short where the copy is.
    const fullW = Math.max(360, window.innerWidth || 1024);
    const contentW = Math.min(fullW, 1080);
    const M = Math.max(17, Math.min(64, Math.round(17 * 1.15 * fullW / contentW)));
    const K = 24, margin = 3;
    let seed = Date.now() >>> 0;
    const rand = () => {
  ...
  // 'I' — the pulse root and the top of the crack graph.
  ...
  // A third hardcoded point on the 'i' of "get in touch" — the bottom anchor of the
  // crack graph (the 'i'). A plain illuminating point otherwise; positioned by layoutDots().
  ...
  window.LITAPI = { incRGB, glowFalloff, GOLD, TEXT_SEL, frameGeom,
  ...
  function drawGlow(ext, cx, cy, rOut, rIn, sx, sy) {
    if (!cctx) return;
  ...
  function draw() {
    ...
    if (igReady) {
      ...
      if (T >= endT) {                                           // sequence over — settle into steady light
        ...
        setCore("");
        crackR = Infinity;                                      // the wave is gone — every crack stays, for good
        ...
        phase = "pulse";
        crackR = Math.max(crackR, IGN.SPEED * ign.Tp);          // the cracks race outward with the front
    ...
    drawGlow(ext, cx, cy, rOut, rIn, sx, sy);
    if (phase === "gather")
    ...
    drawCracks(scale);   // the SPT lightning the front has revealed so far
  ...
  function layoutDots() {
    ...
    const yBot = dotPos[iIndexB] ? dotPos[iIndexB].y : Infinity;
    for (let i = 0; i < dotEls.length; i++) {
  ...
  function contactAnchor() {
    ...
    if (!h || !h.firstChild || h.firstChild.nodeType !== 3) return null;
    const s = h.textContent || "";
    let ci = -1;
    for (let k = 0; k < s.length; k++) { const c = s[k].toLowerCase(); if (c === "i" || c === "í" || c === "ì" || c === "j") { ci = k; break; } }
    if (ci >= 0) {
      const rg = document.createRange(); rg.setStart(h.firstChild, ci); rg.setEnd(h.firstChild, ci + 1);
  ...
  // ---- the crack graph: a directed "final graph" from the 'I' to the 'i' ------------
  // Built ONCE (load / resize), never on a drag — so moving points stays smooth. The full
  // construction is written up in assets/cracks-final-graph.md; in short:
  //  • Bowyer–Watson Delaunay over the in-band dots (the logo-centre dot excluded), pruned
  //    (perimeter sides, and each triangle's longest side when it is > 2× its shortest);
  //  • the shortest path 'I'→'i' is the SPINE;
  //  • from each spine node we grow branches into not-yet-taken points while every step
  //    turns ≤ π/4 from the flow; the 'I' then adopts its nearest leftovers likewise;
  //  • any still-loose point attaches to its nearest already-in node that is closer to 'I'
  //    ("uphill"), so the pulse reveals a connected front.
  // Each edge's CRACK is a scaled copy of the WHOLE graph (self-similar, depth 1), oriented
  // parent(→'I')-to-child and held in a band of half-thickness t — laid down in placeCracks().
  let crackPaths = [];      // self-similar crack sticks: [{ pts:[[ax,ay],[bx,by]], dr:[da,db], spine }]
  let crackR = 0;           // revealed radius from the 'I' (page px); Infinity = all shown
  let graphEdges = [];      // the final graph: [{ p, c }] parent→child dot indices (p closer to 'I')
  let spineKeys = new Set();
  let graphBuilt = false;

  // Delaunay over the in-band dots (logo centre excluded), pruned; adjacency in LOCAL indices.
  function triangulate() {
    const idx = [], pts = [];
    for (let i = 0; i < dotPos.length; i++)
      if (dotPos[i] && i !== centreIndex) { idx.push(i); pts.push([dotPos[i].x, dotPos[i].y]); }
    const n = pts.length;
    if (n < 3) return null;
    let x0 = Infinity, y0 = Infinity, x1 = -Infinity, y1 = -Infinity;
    for (const [x, y] of pts) { if (x < x0) x0 = x; if (y < y0) y0 = y; if (x > x1) x1 = x; if (y > y1) y1 = y; }
    const dm = 10 * Math.max(x1 - x0, y1 - y0) + 10, mx = (x0 + x1) / 2, my = (y0 + y1) / 2;
    pts.push([mx - dm, my - dm], [mx + dm, my - dm], [mx, my + dm]);
    const circum = (i, j, k) => {
    ...
    const mkTri = (i, j, k) => ({ v: [i, j, k], c: circum(i, j, k) });
    let tris = [mkTri(n, n + 1, n + 2)];
    for (let p = 0; p < n; p++) {
      const [px, py] = pts[p], edges = [];
      const bad = tris.filter(t => t.c && (px - t.c.x)**2 + (py - t.c.y)**2 <= t.c.r2);
      for (const t of bad) for (let e = 0; e < 3; e++) edges.push([t.v[e], t.v[(e + 1) % 3]]);
      tris = tris.filter(t => !bad.includes(t));
      for (const [a, b] of edges) {
        let dup = 0;
        for (const [c, d] of edges) if ((c === a && d === b) || (c === b && d === a)) dup++;
        if (dup === 1) tris.push(mkTri(a, b, p));
      }
    }
    const ekey = (a, b) => (a < b ? a * 100000 + b : b * 100000 + a);
    const elen = (a, b) => Math.hypot(pts[a][0] - pts[b][0], pts[a][1] - pts[b][1]);
    const all = new Map(), cut = new Set(), seen = new Map();
    for (const t of tris) {
      if (t.v.some(v => v >= n)) continue;
      const L = [0, 1, 2].map(e => elen(t.v[e], t.v[(e + 1) % 3]));
      const mn = Math.min(L[0], L[1], L[2]), mx2 = Math.max(L[0], L[1], L[2]);
      for (let e = 0; e < 3; e++) {
        const a = t.v[e], b = t.v[(e + 1) % 3], k = ekey(a, b);
        all.set(k, [a, b]); seen.set(k, (seen.get(k) || 0) + 1);
        if (mx2 > 2 * mn && L[e] === mx2) cut.add(k);
      }
    }
    for (const [k, c] of seen) if (c === 1) cut.add(k);
    const adj = Array.from({ length: n }, () => []), adjAll = Array.from({ length: n }, () => []);
    for (const [k, [a, b]] of all) { adjAll[a].push(b); adjAll[b].push(a); if (!cut.has(k)) { adj[a].push(b); adj[b].push(a); } }
    return { n, idx, pts, adj, adjAll, elen };
  }

  function buildGraph() {
    graphBuilt = false; graphEdges = []; spineKeys = new Set();
    const T = triangulate(); if (!T) return;
    const { n, idx, pts, adj, adjAll, elen } = T;
    const loc = new Map(); idx.forEach((di, li) => loc.set(di, li));
    const I = loc.get(iIndex), S = loc.get(iIndexB);
    if (I == null || S == null) return;
    // shortest path 'I'→everything on the PRUNED graph; if the 'i' is unreachable (pruning cut
    // the field in two) fall back to the full Delaunay, which is always connected
    const dijkstra = (A) => {
      const dist = new Array(n).fill(Infinity), par = new Array(n).fill(-1), done = new Array(n).fill(false);
      dist[I] = 0;
      for (;;) {
        let u = -1, best = Infinity;
        for (let k = 0; k < n; k++) if (!done[k] && dist[k] < best) { best = dist[k]; u = k; }
        if (u < 0) break; done[u] = true;
        for (const v of A[u]) { const d = dist[u] + elen(u, v); if (d < dist[v]) { dist[v] = d; par[v] = u; } }
      }
      return par;
    };
    let par = dijkstra(adj);
    if (S !== I && par[S] < 0) par = dijkstra(adjAll);
    if (S !== I && par[S] < 0) return;                          // still unreachable — bail
    const spine = []; { let v = S; while (v >= 0) { spine.push(v); if (v === I) break; v = par[v]; } spine.reverse(); }
    if (spine[0] !== I) return;
    const inFinal = new Set(spine), edges = [];
    const ekey = (a, b) => (a < b ? a * 100000 + b : b * 100000 + a);
    for (let k = 0; k < spine.length - 1; k++) { edges.push([spine[k], spine[k + 1]]); spineKeys.add(ekey(idx[spine[k]], idx[spine[k + 1]])); }
    const QUARTER = Math.PI / 4 + 1e-6;
    const turn = (ux, uy, vx, vy) => {
      const lu = Math.hypot(ux, uy) || 1, lv = Math.hypot(vx, vy) || 1;
      return Math.acos(Math.max(-1, Math.min(1, (ux * vx + uy * vy) / (lu * lv))));
    };
    const grow = (node, dx, dy) => {
      for (const m of adj[node]) {
        if (inFinal.has(m)) continue;
        const mx = pts[m][0] - pts[node][0], my = pts[m][1] - pts[node][1];
        if (turn(dx, dy, mx, my) <= QUARTER) { inFinal.add(m); edges.push([node, m]); grow(m, mx, my); }
      }
    };
    for (let k = spine.length - 2; k >= 1; k--) { const nd = spine[k], pv = spine[k - 1]; grow(nd, pts[nd][0] - pts[pv][0], pts[nd][1] - pts[pv][1]); }
    const iN = adj[I].filter(m => !inFinal.has(m)).sort((a, b) => elen(I, a) - elen(I, b));
    for (const m of iN) { if (inFinal.has(m)) continue; inFinal.add(m); edges.push([I, m]); grow(m, pts[m][0] - pts[I][0], pts[m][1] - pts[I][1]); }
    const dI = k => Math.hypot(pts[k][0] - pts[I][0], pts[k][1] - pts[I][1]);
    for (let pass = 0; pass < n; pass++) {
      let changed = false;
      for (let k = 0; k < n; k++) {
        if (inFinal.has(k)) continue;
        let bestM = -1, bestD = Infinity;
        for (const m of adjAll[k]) if (inFinal.has(m) && dI(m) < dI(k)) { const d = elen(k, m); if (d < bestD) { bestD = d; bestM = m; } }
        if (bestM >= 0) { inFinal.add(k); edges.push([bestM, k]); changed = true; }
      }
      if (!changed) break;
    }
    graphEdges = edges.map(([a, b]) => dI(a) <= dI(b) ? { p: idx[a], c: idx[b] } : { p: idx[b], c: idx[a] });
    graphBuilt = true;
  }
  const spineEdge = e => spineKeys.has(e.p < e.c ? e.p * 100000 + e.c : e.c * 100000 + e.p);

  // t (px): ~half the hero name's letter height, so 2t ≈ its caps — the crack's half-thickness
  function crackT() {
    const nm = document.querySelector(".hero__name");
    return 0.34 * (nm ? parseFloat(getComputedStyle(nm).fontSize) : 40);
  }

  // Lay the self-similar cracks from the fixed graph + live positions. Normalise the graph
  // once (i→origin, I→(1,0), y scaled so max|y| = t), then drop a copy on every edge, oriented
  // parent(→I)-to-child. Arithmetic only — safe to re-run on every drag.
  function placeCracks() {
    crackPaths = [];
    if (!graphBuilt) return;
    const PI = dotPos[iIndex], Pi = dotPos[iIndexB];
    if (!PI || !Pi) return;
    const ox = Pi.x, oy = Pi.y, vx = PI.x - ox, vy = PI.y - oy, D = Math.hypot(vx, vy) || 1;
    const a0 = Math.atan2(vy, vx), ca = Math.cos(-a0), sa = Math.sin(-a0);
    const nodes = new Set(); for (const e of graphEdges) { nodes.add(e.p); nodes.add(e.c); }
    const norm = new Map(); let maxY = 1e-6;
    for (const nd of nodes) {
      const p = dotPos[nd]; if (!p) continue;
      const rx = (p.x - ox) * ca - (p.y - oy) * sa, ry = (p.x - ox) * sa + (p.y - oy) * ca;
      norm.set(nd, [rx / D, ry]); if (Math.abs(ry) > maxY) maxY = Math.abs(ry);
    }
    const ys = crackT() / maxY;
    for (const v of norm.values()) v[1] *= ys;
    for (const e of graphEdges) {
      const A = dotPos[e.p], B = dotPos[e.c]; if (!A || !B) continue;
      const ux = A.x - B.x, uy = A.y - B.y, L = Math.hypot(ux, uy) || 1, pnx = -uy / L, pny = ux / L;
      for (const g of graphEdges) {
        const na = norm.get(g.p), nb = norm.get(g.c); if (!na || !nb) continue;
        const ax = B.x + na[0] * ux + na[1] * pnx, ay = B.y + na[0] * uy + na[1] * pny;
        const bx = B.x + nb[0] * ux + nb[1] * pnx, by = B.y + nb[0] * uy + nb[1] * pny;
        crackPaths.push({ pts: [[ax, ay], [bx, by]],
          dr: [Math.hypot(ax - PI.x, ay - PI.y), Math.hypot(bx - PI.x, by - PI.y)], spine: spineEdge(g) });
      }
    }
  }
  ...
  function buildCracks(rebuild) {
    if (rebuild || !graphBuilt) buildGraph();
    placeCracks();
  }

  const GOLD_A = a => `rgba(${GOLD[0]},${GOLD[1]},${GOLD[2]},${a})`;
  // Paint the revealed cracks up to crackR: gold sticks, the spine-image bolder with a hot
  // core, the growing tips brighter. A handful of batched strokes, so scrolling stays smooth.
  function drawCracks(scale) {
    if (!krctx) return;
    krctx.clearRect(0, 0, crackCanvas._cw, crackCanvas._ch);
    if (!lightsOn || crackR <= 0 || !crackPaths.length) return;
    const sx = window.scrollX, sy = window.scrollY, lead = ign.lead || 80, cw = 0.6 + 0.28 * scale;
    const body = new Path2D(), spine = new Path2D(), front = new Path2D();
    let any = false;
    for (const cp of crackPaths) {
      const da = cp.dr[0], db = cp.dr[1];
      if (da > crackR && db > crackR) continue;
      let a = cp.pts[0], b = cp.pts[1];
      if (db > crackR) { const t = (crackR - da) / (db - da || 1); b = [a[0] + (b[0] - a[0]) * t, a[1] + (b[1] - a[1]) * t]; }
      else if (da > crackR) { const t = (crackR - db) / (da - db || 1); a = [b[0] + (a[0] - b[0]) * t, b[1] + (a[1] - b[1]) * t]; }
      const ax = a[0] - sx, ay = a[1] - sy, bx = b[0] - sx, by = b[1] - sy;
      body.moveTo(ax, ay); body.lineTo(bx, by);
      if (cp.spine) { spine.moveTo(ax, ay); spine.lineTo(bx, by); }
      if (crackR !== Infinity && Math.max(da, db) > crackR - lead) { front.moveTo(ax, ay); front.lineTo(bx, by); }
      any = true;
    }
    if (!any) return;
    krctx.lineCap = "round"; krctx.lineJoin = "round";
    krctx.strokeStyle = GOLD_A(0.14); krctx.lineWidth = cw * 2.2; krctx.stroke(spine);           // spine-image halo
    krctx.strokeStyle = GOLD_A(0.8);  krctx.lineWidth = cw * 0.7; krctx.stroke(body);            // every stick
    krctx.strokeStyle = GOLD_A(0.9);  krctx.lineWidth = cw;       krctx.stroke(spine);           // spine-image, bolder
    krctx.strokeStyle = "rgba(255,244,206,0.9)";  krctx.lineWidth = cw * 0.5; krctx.stroke(spine); // hot core
    krctx.strokeStyle = "rgba(255,250,230,0.95)"; krctx.lineWidth = cw * 1.1; krctx.stroke(front); // growing tips
  }
  ...
  function onMove(e) {
    ...
    if (drag.kind === "dot") {
      ...
      placeCracks();                         // only re-lay the cracks (graph topology is fixed)
    } else {
  ...
  function setLights(on) {
    ...
    if (!on) {
      ...
      setCore("");
      crackR = Infinity;                                    // steady light shows the full crack network
    }
  ...
  function start() {
    ...
    else { for (let i = 0; i < dotEls.length; i++) setLit(i, 1); crackR = Infinity; }   // reduced motion: straight to lit, cracks shown
    draw();
