...
(function () {
  ...
  const centreIndex = dotEls.indexOf(centreDot);   // the logo-centre dot
  const IGN = {
  ...
  function drawLetters(cx, cy, R, toneOut, toneIn) {
    ...
    for (let i = 0; i < chars.length; i++) {
      ...
      if (lvl !== letterLvl[i]) {                            // recolour only when the heat level changes
        // by night the letters die INTO the annulus bone, not black-then-gone: the
        // carved colour blends toward the ring's own ink as the heat drains, so a
        // letter reaches the hide threshold already invisible (no pop when it goes)
        if (nightOn) {
  ...
  function layoutDots() {
    ...
    const anchorB = contactAnchor();
    const yBot = anchorB ? anchorB.y : Infinity;
    for (let i = 0; i < dotEls.length; i++) {
      if (i === iIndex || i === centreIndex) continue;
      const p = dotPos[i]; if (!p) continue;
  ...
  function buildGraph() {
    ...
    const tree = [];                       // grown points; the 'I' stays out (last resort)
    for (const k of rest) {
      let best = -1, ba = Math.PI / 4;     // qualify at π/4 off the radial axis, at most
      for (const m of tree) {
        const ux = dotPos[m].x - PI.x,        uy = dotPos[m].y - PI.y;         // m's radial direction
        const vx = dotPos[k].x - dotPos[m].x, vy = dotPos[k].y - dotPos[m].y;  // the ray m → newcomer
        const du = Math.hypot(ux, uy), dv = Math.hypot(vx, vy);
        if (!du || !dv) continue;
        const ang = Math.acos(Math.max(-1, Math.min(1, (ux * vx + uy * vy) / (du * dv))));
        if (ang <= ba) { ba = ang; best = m; }
      }
      crackEdges.push({ p: best >= 0 ? best : iIndex, c: k });
      tree.push(k);
    }
    const t = crackT();
  ...
  // The crack canvas covers the whole PAGE (not the viewport) and the strokes are
  // painted at their page coordinates: scrolling never enters the paint, so a busy
  // frame can delay the envelope a beat but can never shear the cracks off the page
  ...
  function fitCrackCanvas() {
    ...
    crackCanvas._cw = w; crackCanvas._ch = h;
    crackCanvas.width = Math.round(w * dpr); crackCanvas.height = Math.round(h * dpr);
    crackCanvas.style.width = w + "px"; crackCanvas.style.height = h + "px";
    krctx.setTransform(dpr, 0, 0, dpr, 0, 0);
  }
  ...
  function drawCracks(scale) {
    ...
    if (!live) {
      if (!crackClean) { crackCanvas.width = crackCanvas.height = 0; crackClean = true; }
      return;
    }
    const now = performance.now();
    if (now - crackTick < 31 && crackClean === false) return;
    crackTick = now;
    if (crackCanvas._cw !== document.documentElement.clientWidth ||
        crackCanvas._ch !== document.documentElement.scrollHeight) fitCrackCanvas();
    const PIc = dotPos[iIndex];
    const R = IGN.SPEED * ign.Tp + ign.lead + 20;
    const y0 = PIc ? Math.max(0, PIc.y - R) : 0;
    const y1 = PIc ? Math.min(crackCanvas._ch, PIc.y + R) : crackCanvas._ch;
    krctx.clearRect(0, y0, crackCanvas._cw, y1 - y0);
    crackClean = false;
    const cw = 0.45 + 0.2 * scale;                // a fine fissure, not a wire
    ...
    for (const g of crackSegs) {
      ...
      P.moveTo(g.ax, g.ay); P.lineTo(g.bx, g.by);
    }
    krctx.lineCap = "round"; krctx.lineJoin = "round";
    for (const [wGrow, aScale] of [[1.1, 0.3], [0, 0.75]]) {   // halo, then core
      for (let w = 0; w < 3; w++) {
        krctx.lineWidth = cw * CRACK_W[w] + wGrow;
        for (let lv = 1; lv <= CRACK_LVL; lv++) {
          if (!bins[w][lv]) continue;
          const a = CRACK_A[w] * aScale * lv / CRACK_LVL;
          const c = incRGB(lv / CRACK_LVL);   // the ember ramp, by envelope level
          krctx.strokeStyle = `rgba(${c[0]},${c[1]},${c[2]},${a.toFixed(3)})`;
  ...
  // ---- night mode: the palette swap, seated between bulb and pill ----
  // Night re-dresses the page UNDER the lights and touches nothing above them: the
  // CSS carries the palette (body.night). Toggling also re-reads the mark ink the
  // ring letters must die into (see drawLetters), and forces their recolour.
  ...
  function setNight(on) {
    ...
    if (nightBtn) {
      ...
      nightBtn.setAttribute("aria-label", on ? "Switch to day mode" : "Switch to night mode");
    }
    // the annulus ink under the carved letters, for their night fade-out
    const m = getComputedStyle(document.body).getPropertyValue("--logo-ink").match(/[0-9a-f]{6}/i);
    NIGHT_INK = m ? [0, 1, 2].map(i => parseInt(m[0].substr(i * 2, 2), 16)) : [236, 229, 212];
    letterLvl.fill(-1);                      // stale colours: repaint every warm letter
    remember("night", on ? "on" : "off");
    schedule();
  }
