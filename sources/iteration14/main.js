...
(function () {
  ...
  const FONT_FRAC = 0.32;             // letter font px as a fraction of the ring thickness (smaller again — the message should stay subtle)
  ...
  // A point's omni glow, but with alpha falling QUADRATICALLY with radius (1 − s²): nearly
  // flat and bright through the core, then dropping off fast toward the rim — it reads as
  // real light spilling out, not a flat circular filter that just thins with distance.
  // Sampled into stops, since a canvas gradient only interpolates linearly between them.
  function radialQuad(c, x, y, R, rgb, peak) {
    const g = c.createRadialGradient(x, y, 0, x, y, R), S = 8;
    for (let i = 0; i <= S; i++) {
      const s = i / S;
      g.addColorStop(s, `rgba(${rgb[0]},${rgb[1]},${rgb[2]},${(peak * (1 - s * s)).toFixed(3)})`);
    }
    return g;
  }
  function drawCast(ext, cx, cy, rOut, rIn, scale) {
    ...
    for (const q of ext) {
      ...
      const glowW = 1 - coneW;
      if (glowW > 0.01) {                          // far: a soft omni glow around the point (quadratic falloff)
        const a = (CAST_ALPHA * 1.6 * glowW * w);
        cctx.fillStyle = radialQuad(cctx, q.px, q.py, glowR, CAST_GLOW, a);
  ...
  function drawPreambleGlow(icx, icy, prog, glowR, Rpage) {
    ...
    cctx.fillStyle = radialQuad(cctx, icx, icy, radius, CAST_GLOW, peak);
  ...
  function draw() {
    ...
    if (!lightsOn) {                       // master light off: freeze the logo upright, wipe the lit layers
      ...
      if (lctx) lctx.clearRect(0, 0, lettersCanvas._cw || 0, lettersCanvas._ch || 0);
  ...
  // bulb: off removes the points + every lighting layer (CSS) and freezes the logo upright;
  // on returns straight to steady light (no re-ignition)
  ...
  // language: a first-draft Spanish version, swapped in element by element. Each translatable
  // node carries data-i18n="key"; its English is the markup already in the page, captured here.
  // Proper names (the hero name, degree names already in Spanish, paper titles) are left alone.
  const ES = {
    skip: "Saltar al contenido",
    eyebrow: "Matemático · Investigador predoctoral",
    lede: "Trabajo donde la geometría se encuentra con los datos: detectar estructura en nubes de puntos y razonar con cuidado por qué un método funciona, antes de fiarme de que lo hace. Afincado en Galicia, España.",
    cv: "Currículum (PDF)",
    k_about: "Sobre mí",
    ...
    lang_en: "Inglés",
    lvl_native: "Nativo",
    lvl_toefl: "TOEFL · fluido",
    sk_focus: "Áreas de interés",
    chip_pcg: "Geometría de nubes de puntos",
    chip_det: "Métodos de detección",
    chip_harm: "Análisis armónico",
    chead: "Hablemos",
  ...
  if (langBtn) {
    ...
    const EN = {}; nodes.forEach(n => EN[n.dataset.i18n] = n.innerHTML);
    ...
    function setLang(l) {
      ...
      for (const n of nodes) {
        ...
        n.innerHTML = l === "es" ? (ES[k] != null ? ES[k] : EN[k]) : EN[k];
      }
      document.title = l === "es" ? ES._title : enTitle;
      if (meta) meta.content = l === "es" ? ES._desc : enDesc;
      layoutDots(); draw();                                 // text reflow shifts the page-anchored dots
    }
    langBtn.addEventListener("click", () => setLang(lang === "en" ? "es" : "en"));
  }

  // the controls belong to the top of the page — fade them out once you scroll into the content
  function syncControls() {
    if (controls) controls.classList.toggle("controls--hidden", window.scrollY > 160);
  }
