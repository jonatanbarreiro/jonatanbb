...
(function () {
  ...
  const PRIM = [[25.5,84.5],[19,91],[19,56],[49.5,86.5],[0,136],[0,0],[4.3,7.6]];
  const USES = [[44.6,81.8],[119,57.75]];
  const KLOGO = 1.05309;                            // logo scale (miter tips at 95% of inner radius)
  const HW = 10.5 * KLOGO / 2;                      // half stroke, viewBox units
  const gX = x => 200 + KLOGO * (x - 76.55);        // the group transform (MEC-of-miter-tips centred)
  const gY = y => 200 + KLOGO * (y - 134.1421);
  ...
  // incidence angle β at the inner-boundary piece at angle `a` from light `p`
  // (p, piece both relative to the circle centre): cos β = L·n, n = inward normal
  function innerBeta(a, p, rIn) {
    const Px = rIn * Math.cos(a), Py = rIn * Math.sin(a);
    const lx = p.d * Math.cos(p.th) - Px, ly = p.d * Math.sin(p.th) - Py;
    const ll = Math.hypot(lx, ly) || 1;
    const c = (lx * -Math.cos(a) + ly * -Math.sin(a)) / ll;
    return Math.acos(Math.max(-1, Math.min(1, c)));
  }

  // illuminance → displayed alpha. A gradual curve (slow to saturate) so the
  // bright/dim fluctuation stays visible to the eye even where points overlap.
  // Gain is high because the energy-normalized illuminances are small.
  const tone = v => 1 - Math.exp(-2.6 * v);

  // Draw one boundary band as a FILLED, variable-width ring: anchored at radius
  // `r0`, reaching `dir` (±1) into the ring by an amount that grows with the
  // illuminance — so a bright piece reaches across toward the other boundary and
  // a piece that's just-not-lit has zero width (smooth pointy ends). The alpha is
  // a per-angle conic gradient. `f(a)` returns the raw illuminance at angle a.
  function band(b, r0, dir, f) {
    ...
    octx.beginPath();
    for (let k = 0; k <= N; k++) {                          // anchored edge (r0)
      const a = (k / N) * TAU, x = b.cx + r0 * Math.cos(a), y = b.cy + r0 * Math.sin(a);
      k ? octx.lineTo(x, y) : octx.moveTo(x, y);
    }
    for (let k = N; k >= 0; k--) {                          // reaching edge (r0 + dir*reach)
      const a = (k / N) * TAU, r = r0 + dir * reach[k];
  ...
  // Energy-normalised: every point has total energy 1. An exterior point at
  // distance d sends only the fraction arcsin(r/d)/π of its light to the circle
  // (the share of directions the circle blocks); an interior point sends all of
  // it (1), spread over the whole inner boundary. The angular *shape* is the
  // existing linear approximation; we just rescale each point so its integral
  // over the boundary equals that energy.
  const NB = 220, TAU = 2 * Math.PI;
  function drawBands(b) {
    if (b.ex.length) {                                   // outer: external points, reaching inward
      band(b, b.rOut, -1, a => {
        let v = 0;
        for (const q of b.ex) {
          const ratio = b.rOut / q.d; if (ratio >= 1) continue;
          const gamma = Math.acos(ratio); if (gamma < 0.12) continue;   // skip near-boundary spike
          let d = a - q.th; d = Math.atan2(Math.sin(d), Math.cos(d));
          if (Math.abs(d) < gamma) {
            const E = Math.asin(ratio) / Math.PI;        // energy reaching the circle
            v += (E / gamma) * (1 - Math.abs(d) / gamma); // ∫ over arc = E
          }
        }
        return v;
      });
    ...
    if (b.inn.length) {                                  // inner: internal points, reaching outward
      for (const p of b.inn) {                           // per-point max β, then ∫shape (to normalise to 1)
        let bmax = 0;
        for (let k = 0; k < NB; k++) { const be = innerBeta((k / NB) * TAU, p, b.rIn); if (be > bmax) bmax = be; }
        p.bmax = bmax || 1;
        let I0 = 0;
        for (let k = 0; k < NB; k++) I0 += 1 - 0.5 * (innerBeta((k / NB) * TAU, p, b.rIn) / p.bmax);
        p.I0 = (I0 * TAU / NB) || 1;
      }
      band(b, b.rIn, +1, a => {
        let v = 0;
        for (const p of b.inn) v += (1 - 0.5 * (innerBeta(a, p, b.rIn) / p.bmax)) / p.I0;   // ∫ = 1
        return v;
      });
    }
  }
  ...
  function draw() {
    ...
    // the thickened circumference: outer/inner boundary radii. Its stroke-width
    // (11.0574) equals the logo's, i.e. half = 5.5287 each side.
    const halfStroke = 5.5287 * scale, rOut = R + halfStroke, rIn = R - halfStroke;
    ...
    // one illuminating point hardcoded at the centre of the circumference: an
    // interior light, so it lights the inner boundary (and the logo from within)
    innerLights.push({ th: 0, d: 0 });
    interiorLights.push({ x: cx, y: cy, w: 1 });
    ...
    drawReflections(interiorLights,
      { ex: exteriorLights, inn: innerLights, cx, cy, rOut, rIn,
        maxReach: 0.595 * (rOut - rIn) });          // reaches rescaled to 0.7 of the previous baseline
  }

  let raf = 0;
  const schedule = () => { if (!raf) raf = requestAnimationFrame(() => { raf = 0; draw(); }); };
  window.addEventListener("scroll", schedule, { passive: true });
  window.addEventListener("resize", () => { sizeCanvas(); schedule(); });
