...
(function () {
  ...
  const HEAT = [
    ...
    [0.72, 198, 46, 44],   // bright crimson
    [1.00, 232, 53, 32]    // hot escarlata (was orange 242,130,48 — too warm)
  ];
  function incRGB(t) {
  ...
  function lightAndPaintB(cx, cy, scale, cR, sR, inn) {
    ...
    const ONEL_REACH = 31.5 * scl;   // a light behind an element heats only within this radius of itself
    ...
    for (const p of inn) {
      if (p.role === 'region' && p.ridx >= 0) {
        ...
        // a light behind an element heats it only NEAR the light — a hotspot that fades
        // to nothing within one annulus thickness (ONEL_REACH) along the face, like a
        // spot of metal warming under it. Painted on the OUTER (silhouette) face, the one
        // geometry defined for ALL 18 elements (B_EL leaves the 6 region-dark faces empty).
        const A = FA[p.bk][1][p.eidx];
        if (A) {
          const B = FB[p.bk][1][p.eidx], base = bhBase(p.bk, 1, p.eidx);
          const ax = A[0], ay = A[1], ex = B[0]-ax, ey = B[1]-ay;
          for (let j = 0; j <= 10; j++) {
            const X = ax + ex*j/10, Y = ay + ey*j/10, d = Math.hypot(p.px - X, p.py - Y);
            if (d < ONEL_REACH) bRaw[base + j] += 6 * (1 - d / ONEL_REACH);  // hot at the point, gone by one thickness
          }
        }
      } else if (p.role === 'ext') {
        for (let b = 0; b < 2; b++) {
